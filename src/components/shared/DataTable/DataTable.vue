<template>
    <div class="main_table">
        <!--*********** Data Table ***********-->
        <DataTable :class="styleTable" v-if="!loading" :value="products" removableSort :filters="filters" :sortOrder="-1"
            :rows="8">
                ...
                <template #body="slotProps" v-if="typeof col.field.split(',') == 'object'">
                    <div class="d-flex-c gap-3">
                        <div v-for="(fieldItem, i) of col.field.split(',')" :key="fieldItem" class="col_div"
                            ...
                            <span v-if="col.field.split(',').includes('currency') && slotProps.data[fieldItem]"
                                class="me-1">
            ...
            <Column :header="selectTable.header" v-if="selectTable">
                <template #body="slotProps">
                    <DataTableSelect class="mx-auto" :productId="slotProps.data['id']" @updateType="updateType" />
                </template>
            </Column>
            ...
            <Column v-if="quantity">
                <template #body="slotProps">
                    <DataTableQuantity class="mx-auto" :productId="slotProps.data['id']"
